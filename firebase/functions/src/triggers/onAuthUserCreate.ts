...
async function getSignupBonusTokens(): Promise<number> {
  try {
...
export const onAuthUserCreate = functions.auth.user().onCreate(async (user) => {
  ...
  if (bonusTokens <= 0) {
    functions.logger.info("Signup bonus disabled, skipping token grant", { uid, bonusTokens });
    return null;
  }
  ...
  functions.logger.info("Granted signup token bonus", { uid, bonusTokens, txId });
  return null;
});
