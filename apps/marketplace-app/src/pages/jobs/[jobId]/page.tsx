...
export default function CustomerJobDetail() {
  ...
  const showLiveTracking = job.status !== "completed" && job.status !== "cancelled";
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-6 border border-purple-100">
          <div className="p-6 border-b border-gray-100">
            <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2">
              ...
              {showLiveTracking ? "Live Tracking" : "Trip Receipt Map"}
            </h3>
          </div>
          <MapboxMap
            pickup={job.pickup}
            dropoff={job.dropoff}
            courierLocation={showLiveTracking ? courier?.location || null : null}
            pickupProof={job.pickupProof || null}
            dropoffProof={job.dropoffProof || null}
            height="500px"
          />
          {showLiveTracking && job.courierUid && !courier?.location && (
