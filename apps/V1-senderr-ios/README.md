...
- Workspace: `apps/V1-senderr-ios/ios/Senderrappios.xcworkspace`
- Scheme: `Senderr`
...
```bash
open apps/V1-senderr-ios/ios/Senderrappios.xcworkspace
```
...
- Type-check gate is available and should pass for architecture changes:
  - `pnpm --filter v1-senderr-ios exec tsc --noEmit`
...
- Ports:
  - `apps/V1-senderr-ios/src/services/ports/authPort.ts`
  - `apps/V1-senderr-ios/src/services/ports/jobsPort.ts`
  - `apps/V1-senderr-ios/src/services/ports/locationPort.ts`
  - `apps/V1-senderr-ios/src/services/ports/notificationsPort.ts`
  - `apps/V1-senderr-ios/src/services/ports/analyticsPort.ts`
- Adapters:
  - `apps/V1-senderr-ios/src/services/adapters/*`
- Registry provider:
  - `apps/V1-senderr-ios/src/services/serviceRegistry.tsx`
...
- `No Podfile found`:
  - Ensure you are in `apps/V1-senderr-ios/ios`.
- Device cannot reach Metro on `localhost:8081`:
  - Start Metro on LAN host:
    - `cd apps/V1-senderr-ios && npx react-native start --host 0.0.0.0 --port 8081 --reset-cache`
  - Keep Mac and phone on same Wi-Fi.
- Firebase crash for missing plist:
  - Ensure file exists at:
    - `apps/V1-senderr-ios/ios/Senderrappios/GoogleService-Info.plist`
