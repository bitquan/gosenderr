...
export default function RateCardsPage() {
  ...
  const [packageRateCard, setPackageRateCard] = useState<PackageRateCard | null>(null);
    ...
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      ...
      if (userDoc.exists()) {
        ...
        if (courierProfile) {
          ...
          setPackagesEnabled(courierProfile.workModes?.packagesEnabled || false);
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
          <div>
            ...
            {packageRateCard ? (
              ...
              <p style={{ color: '#6b7280', fontSize: '14px' }}>No rate card set up yet</p>
            )}
          </div>
          <button
          ...
          <div style={{ padding: '16px', background: '#f9fafb', borderRadius: '8px' }}>
            <div style={{ fontSize: '14px', color: '#374151' }}>
              <p style={{ marginBottom: '8px' }}>
                <strong>Current Rate:</strong> ${packageRateCard.baseFare.toFixed(2)} base + 
                ${packageRateCard.perMile.toFixed(2)}/mi + ${packageRateCard.perMinute.toFixed(2)}/min
              </p>
              {packageRateCard.optionalFees && packageRateCard.optionalFees.length > 0 && (
                <p style={{ marginBottom: '0' }}>
                  <strong>Optional Fees:</strong>{' '}
                  {packageRateCard.optionalFees.map((fee) => `${fee.name} ($${fee.amount.toFixed(2)})`).join(', ')}
                </p>
              )}
            </div>
          </div>
        ...
        {foodRateCard && (
          <div style={{ padding: '16px', background: '#fef3c7', borderRadius: '8px' }}>
            <div style={{ fontSize: '14px', color: '#78350f' }}>
              <p style={{ marginBottom: '8px' }}>
                <strong>Current Rate:</strong> ${foodRateCard.baseFare.toFixed(2)} base + 
                ${foodRateCard.perMile.toFixed(2)}/mi + ${foodRateCard.restaurantWaitPay.toFixed(2)}/min wait
              </p>
              {foodRateCard.peakHours && foodRateCard.peakHours.length > 0 && (
                <p style={{ marginBottom: '8px' }}>
                  <strong>Peak Hours:</strong>{' '}
                  {foodRateCard.peakHours.map((peak) => 
              ...
              {foodRateCard.optionalFees && foodRateCard.optionalFees.length > 0 && (
                <p style={{ marginBottom: '0' }}>
                  <strong>Optional Fees:</strong>{' '}
                  {foodRateCard.optionalFees.map((fee) => `${fee.name} ($${fee.amount.toFixed(2)})`).join(', ')}
                </p>
              )}
            </div>
          </div>
