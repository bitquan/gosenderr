...
import { ServiceLevel, HubDoc, PackageJourneyLeg, LegType, FeatureFlags } from '@gosenderr/shared';
...
function calculatePricing(
  ...
  const subtotal = (localPickup + localDelivery + longHaulTotal) * serviceLevelMultiplier;
...
export default function ShipPage() {
  ...
  const findNearestHub = (lat: number, lng: number): { hub: HubDoc; distance: number } | null => {
    if (hubs.length === 0) return null;
    
    let nearest: { hub: HubDoc; distance: number } | null = null;
    for (const hub of hubs) {
      const distance = calculateDistance(lat, lng, hub.location.lat, hub.location.lng);
  ...
  const handleFromAddressChange = (address: string, lat: number, lng: number) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!packageDetails.weight || !packageDetails.length || !packageDetails.width || 
        !packageDetails.height || !packageDetails.declaredValue) {
    ...
    try {
      ...
      const packageDoc = {
        ...
        stripePaymentIntentId: '', // Would be set after payment processing
      ...
      router.push(`/customer/packages/${docRef.id}`);
    } catch (err) {
  ...
  const longHaulLegs = journey.filter(leg => leg.type === 'long_haul').length;
  const pricing = fromAddress && toAddress ? calculatePricing(
    fromAddress.distanceToHub || 0,
    toAddress.distanceToHub || 0,
    longHaulLegs,
    serviceLevel
  ...
  return (
    <div className="container" style={{ maxWidth: '900px', margin: '0 auto', padding: '40px 20px' }}>
      <h1 style={{ textAlign: 'center', marginBottom: '32px' }}>Ship a Package</h1>
      ...
      <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            {fromAddress?.nearestHub && (
              <div style={{
                ...
                📍 Nearest Hub: {fromAddress.nearestHub.name} ({fromAddress.distanceToHub?.toFixed(1)} miles away)
              </div>
            )}
          </div>
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <input
              ...
              onChange={e => {
                ...
                const parts = e.target.value.split(',');
                if (parts.length >= 2) {
                  handleToAddressChange(e.target.value, 40.7128, -74.0060);
                }
              }}
              style={{
                width: '100%',
            ...
            {toAddress?.nearestHub && (
              <div style={{
                ...
                📍 Nearest Hub: {toAddress.nearestHub.name} ({toAddress.distanceToHub?.toFixed(1)} miles away)
              </div>
            )}
          </div>
        </GlassCard>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
            ...
            <div>
              ...
              <input
                ...
                onChange={e => setPackageDetails({ ...packageDetails, declaredValue: e.target.value })}
                style={{
                  width: '100%',
                  padding: '10px',
                  borderRadius: '8px',
        ...
        <GlassCard style={{ marginBottom: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
            {(['standard', 'express', 'priority'] as ServiceLevel[]).map(level => (
              <button
                key={level}
                type="button"
                ...
                style={{
                  padding: '16px',
                  backgroundColor: serviceLevel === level ? '#dbeafe' : '#f9fafb',
                  border: serviceLevel === level ? '2px solid #3b82f6' : '2px solid #e5e7eb',
                  borderRadius: '8px',
                  cursor: 'pointer',
                  textAlign: 'center',
                }}
                ...
                <div key={index} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '13px', color: '#6b7280' }}>
                      {leg.type === 'local_pickup' && `To ${fromAddress?.nearestHub?.name}`}
                      {leg.type === 'long_haul' && `${fromAddress?.nearestHub?.name} → ${toAddress?.nearestHub?.name}`}
                      {leg.type === 'local_delivery' && `From ${toAddress?.nearestHub?.name}`}
                    </div>
                  </div>
                </div>
              ))}
        ...
        {pricing && (
          <GlassCard style={{ marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '16px 0', fontSize: '20px', fontWeight: 600 }}>
                <span>Total</span>
        ...
        <button
          ...
          style={{
            width: '100%',
            padding: '16px',
            backgroundColor: submitting || !fromAddress || !toAddress ? '#9ca3af' : '#3b82f6',
            color: 'white',
            border: 'none',
            borderRadius: '8px',
            fontSize: '18px',
            fontWeight: 600,
            cursor: submitting || !fromAddress || !toAddress ? 'not-allowed' : 'pointer',
          }}
        >
