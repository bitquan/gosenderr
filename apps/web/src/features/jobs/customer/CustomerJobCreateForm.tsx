...
export function CustomerJobCreateForm({ uid }: CustomerJobCreateFormProps) {
  ...
  const [tempJobId] = useState(() => `temp_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`);
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto' }}>
      ...
      <form onSubmit={handleSubmit} style={{ background: 'white', padding: '24px', borderRadius: '8px', border: '1px solid #ddd' }}>
        <div style={{ marginBottom: '24px' }}>
          <AddressAutocomplete
            label="Pickup Address"
            placeholder="Enter pickup address"
            onSelect={(result) => {
              setPickup({ lat: result.lat, lng: result.lng, label: result.address });
            }}
          />
        </div>
        ...
        <div style={{ marginBottom: '24px' }}>
          <AddressAutocomplete
            ...
            onSelect={(result) => {
              setDropoff({ lat: result.lat, lng: result.lng, label: result.address });
            }}
          />
        </div>
        ...
        {pickup && dropoff && (
          ...
          >
            ...
            {estimateSource === 'couriers' && (
              <div style={{ fontSize: '12px', color: '#666', marginTop: '4px' }}>
                {couriers.filter(c => c.eligible).length} eligible courier(s) nearby
              </div>
            )}
            {couriersLoading && (
