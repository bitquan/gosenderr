...
export function JobForm({ onSubmit, loading = false, onPickupDropoffChange }: JobFormProps) {
  ...
  const notifyChange = () => {
    if (onPickupDropoffChange) {
      ...
      const pickup = (!isNaN(pLat) && !isNaN(pLng)) ? { lat: pLat, lng: pLng } : null;
      const dropoff = (!isNaN(dLat) && !isNaN(dLng)) ? { lat: dLat, lng: dLng } : null;
