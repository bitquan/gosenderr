    ...
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      >
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            {estimatedArrivalMinutes !== undefined && status !== 'completed' && (
      ...
      {courierInfo && (
        ...
        >
          ...
          {courierInfo.vehicleDetails && (
            ...
            >
              🚗 {courierInfo.vehicleDetails.color} {courierInfo.vehicleDetails.make}{' '}
          ...
          {courierLocation && (
            ...
            >
              📍 Last updated: {lastUpdateSeconds < 60 ? `${lastUpdateSeconds} seconds ago` : `${Math.floor(lastUpdateSeconds / 60)} minutes ago`}
            </div>
          )}
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {/* Picked Up */}
          <TimelineItem
            icon={pickupPhoto ? '📸' : status === 'picked_up' || status === 'enroute_dropoff' || status === 'arrived_dropoff' || status === 'completed' ? '✅' : '⏳'}
            title="Picked Up"
            subtitle={pickupPhoto ? 'View Pickup Photo' : undefined}
            completed={status === 'picked_up' || status === 'enroute_dropoff' || status === 'arrived_dropoff' || status === 'completed'}
          ...
          {/* In Transit */}
          <TimelineItem
            icon={status === 'enroute_dropoff' || status === 'arrived_dropoff' || status === 'completed' ? '🔵' : '⏳'}
            title="In Transit"
            subtitle={
              estimatedArrivalMinutes !== undefined && status === 'enroute_dropoff'
                ? `${estimatedArrivalMinutes} min away`
                : undefined
            }
            completed={status === 'completed'}
            current={status === 'enroute_dropoff' || status === 'arrived_dropoff'}
            lastUpdate={courierLocation ? `Last updated: ${lastUpdateSeconds}s ago` : undefined}
          />
...
function ProofPhotoCard({ title, photo, address }: ProofPhotoCardProps) {
  return (
    <div>
      <div style={{ fontWeight: '600', marginBottom: '8px', fontSize: '14px' }}>{title}</div>
      <div
        style={{
          position: 'relative',
          borderRadius: '8px',
