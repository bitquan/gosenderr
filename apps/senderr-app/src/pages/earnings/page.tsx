...
export default function EarningsPage() {
  ...
  const walletVisible = (userDoc as any)?.courierProfile?.showTokenWallet !== false;
  ...
  const payoutsEnabled = Boolean(
    (userDoc as any)?.courierProfile?.stripePayoutsEnabled ||
      (userDoc as any)?.stripePayoutsEnabled,
  );
  const chargesEnabled = Boolean(
    (userDoc as any)?.courierProfile?.stripeChargesEnabled ||
      (userDoc as any)?.stripeChargesEnabled,
  );
  const providerAccountId =
    (userDoc as any)?.courierProfile?.stripeConnectAccountId ||
    (userDoc as any)?.courierProfile?.stripeAccountId ||
    null;
  const requirementsDue =
    ((userDoc as any)?.courierProfile?.stripeRequirementsDue as string[] | undefined) ||
    [];
  const requirementsPastDue =
    ((userDoc as any)?.courierProfile?.stripeRequirementsPastDue as string[] | undefined) ||
    [];
    ...
    <div className="min-h-screen bg-[#F8F9FF]">
      <div className="max-w-4xl mx-auto p-4 sm:p-6">
        ...
        <Card variant="elevated" className="mb-6">
          <CardHeader>
            <CardTitle>Payout Center</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
              <div className="rounded-xl border border-gray-200 p-3">
                <p className="text-xs text-gray-500">Payout Mode</p>
              ...
              <div className="rounded-xl border border-gray-200 p-3">
                <p className="text-xs text-gray-500">Payout rails</p>
                <p className="text-sm font-semibold text-gray-900 mt-1">
                  {payoutsEnabled ? "Ready" : "Setup needed"}
                </p>
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              <Link
                to="/settings"
                className="px-3 py-2 rounded-lg bg-gray-100 text-sm font-semibold text-gray-700 hover:bg-gray-200"
              >
                Payout preferences
              </Link>
              <Link
                to="/onboarding/stripe"
                className="px-3 py-2 rounded-lg bg-indigo-600 text-sm font-semibold text-white hover:bg-indigo-700"
              >
                Provider setup
              </Link>
            </div>
            <div className="mt-4 rounded-xl border border-gray-200 bg-gray-50 p-3 text-sm text-gray-700">
              <p className="font-semibold text-gray-900">Provider readiness</p>
              <div className="mt-2 grid sm:grid-cols-3 gap-2 text-xs">
                <div>
                  <p className="text-gray-500">Account</p>
                  <p className="font-medium text-gray-900">
                    {providerAccountId ? `Connected (${String(providerAccountId).slice(0, 8)}…)` : "Not connected"}
                  </p>
                </div>
                <div>
                  <p className="text-gray-500">Requirements due</p>
                  <p className="font-medium text-gray-900">{requirementsDue.length}</p>
                </div>
                <div>
                  <p className="text-gray-500">Past due</p>
                  <p className="font-medium text-gray-900">{requirementsPastDue.length}</p>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <div className="mt-6 p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
          <p className="text-sm text-emerald-800">
            <strong>💡 Tip:</strong> Payouts follow your selected payout mode.
            Keep your provider setup current and your wallet visibility preference aligned with your workflow.
          </p>
        </div>
      </div>
    </div>
