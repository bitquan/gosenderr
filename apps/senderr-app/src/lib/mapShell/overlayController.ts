...
export type LocationSnapshot = { latitude: number; longitude: number } | null;
...
type DeriveMapShellStateInput = {
  ...
  courierLocation: LocationSnapshot;
  tracking: boolean;
};
...
const hasArrived = (
  ...
  if (!courierLocation || !targetLocation) return false;
  return (
    distanceMeters(courierLocation, targetLocation) <= ARRIVAL_RADIUS_METERS
  );
};
...
export const deriveMapShellState = ({
  ...
  if (isSyncDegraded(jobsSyncState)) return "offline_reconnect";
