...
2. **Gate 1 (10%):** monitor crash-free rate and map-shell action completion.
3. **Gate 2 (50%):** verify offline/reconnect recovery metrics remain stable.
4. **Gate 3 (100%):** switch to map-shell default; keep tab-shell fallback for one release window.
...
1. Toggle `mapShell` off remotely.
2. Keep `mapRouting`/`jobStatusActions` flags unchanged unless they are the incident source.
3. Capture incident summary + timeline in `docs/dev/WORKLOG.md`.
